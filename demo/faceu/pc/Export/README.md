...
本目录提供 Windows 导出的运行时依赖及接口头文件，核心 API 位于 `core/include/HJRenderContextExport.h`、`core/include/HJRenderFaceuExport.h`。

## 头文件与库

- 头文件：`core/include`
- 动态库：`core/HJRender.dll`（导入库 `core/HJRender.lib`）
- 当前提供为MD动态库 Release
...
| 函数                                                                                                                                              | 说明                                                                              | 备注                                                                                                                                          |
| ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `int renderContextInit(const HJEntryContextInfo& ctx);`                                                                                         | 初始化渲染全局上下文（日志等），成功返回 0。                                                         | 全局仅调用一次，必须主线程，因为涉及到windows window句柄的使用                                                                                                      |
| `void renderContextUnInit();`                                                                                                                   | 释放渲染上下文。                                                                        | 退出前调用                                                                                                                                       |
| `void* faceuInit(HJFaceuCallback cb, bool useEnv = false);`                                                                                      | 创建 Faceu 句柄并注册回调；`useEnv=true` 时使用EGL环境模式。返回句柄，失败返回 `nullptr`。                     | 此函数涉及到opengl操作，如果上层需要context，注意保存，调用完毕后恢复，另外关于faceuInit, faceuAdd, faceuRemove,faceuRemoveAll, faceuRender,faceuDone均涉及到opengl操作，应在一个线程中调用； |
...
void onFaceuNotify(const char* msg, int type) {
    // 处理完成
}

void* h = faceuInit(onFaceuNotify, true);
