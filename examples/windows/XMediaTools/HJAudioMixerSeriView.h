...
class HJAudioMixerSeriView : public HJView
{
public:
    HJ_DECLARE_PUWTR(HJAudioMixerSeriView);

    explicit HJAudioMixerSeriView();
    virtual ~HJAudioMixerSeriView();
...
private:
    ...
    int prepareSession();
    int prepareInputPlayer(size_t i_index);
    int playInput(size_t i_index);
    int pauseInput(size_t i_index);
    int stopInput(size_t i_index);
    int startMixer();
    int stopMixer();
    void stopSession();
