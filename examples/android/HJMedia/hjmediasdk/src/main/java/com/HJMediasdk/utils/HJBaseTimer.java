 ...
 */
/**
 * Timers schedule one-shot or recurring {@link HJBaseTimerTask tasks} for execution.
 ...
 * synchronization.
 */
public class HJBaseTimer {

    private class sTimerImpl extends Thread {

        private class TimerHeap {
            private int DEFAULT_HEAP_SIZE = 256;

            private HJBaseTimerTask[] timers = new HJBaseTimerTask[DEFAULT_HEAP_SIZE];

            private int size = 0;

            private int deletedCancelledNumber = 0;

            public HJBaseTimerTask minimum() {
                return timers[0];
            }
            ...
            public void insert(HJBaseTimerTask task) {
                if (timers.length == size) {
                    HJBaseTimerTask[] appendedTimers = new HJBaseTimerTask[size * 2];
            ...
            private void upHeap() {
                ...
                while (timers[current].when < timers[parent].when) {
                    // swap the two
                    HJBaseTimerTask tmp = timers[current];
            ...
            public void reset() {
                timers = new HJBaseTimerTask[DEFAULT_HEAP_SIZE];
                size = 0;
            }
            ...
            private int getTask(HJBaseTimerTask task) {
         ...
         */
        sTimerImpl(String name, boolean isDaemon) {
            this.setName(name);
        ...
        private void insertTask(HJBaseTimerTask newTask) {
...
//        private final sTimerImpl impl;
//
//        FinalizerHelper(sTimerImpl impl) {
//            this.impl = impl;
//        }
//
...
//    }
    ...
    private sTimerImpl impl = null;

    // Used to finalize thread
    //@SuppressWarnings("unused")
    //private FinalizerHelper finalizer;

    public HJBaseTimer(String name, boolean isDaemon)
    {
        if (name == null) {
            throw new NullPointerException("name == null");
        }
        this.impl = new sTimerImpl(name, isDaemon);
     ...
     */
    public HJBaseTimer(String name) {
        this(name, false);
    }
     ...
     */
    public HJBaseTimer(boolean isDaemon) {
        this("Timer-" + HJBaseTimer.nextId(), isDaemon);
    }
     ...
     */
    public void schedule(HJBaseTimerTask task, Date when) {
        if (when.getTime() < 0) {
     ...
     *                scheduled or canceled.
     */
    public void schedule(HJBaseTimerTask task, long delay) {
        if (delay < 0) {
     ...
     *                scheduled or canceled.
     */
    public void schedule(HJBaseTimerTask task, long delay, long period) {
        if (delay < 0 || period <= 0) {
     ...
     *                scheduled or canceled.
     */
    public void schedule(HJBaseTimerTask task, Date when, long period) {
     ...
     *                scheduled or canceled.
     */
    public void scheduleAtFixedRate(HJBaseTimerTask task, long delay, long period) {
        if (delay < 0 || period <= 0) {
            throw new IllegalArgumentException();
        }
     ...
     *                scheduled or canceled.
     */
    public void scheduleAtFixedRate(HJBaseTimerTask task, Date when, long period) {
        if (period <= 0 || when.getTime() < 0) {
            throw new IllegalArgumentException();
        }
        long delay = when.getTime() - System.currentTimeMillis();
     ...
     * Schedule a task.
     */
    private void scheduleImpl(HJBaseTimerTask task, long delay, long period, boolean fixed) {
        synchronized (impl) {
            if (impl.cancelled) {
            ...
            if (when < 0) {
                throw new IllegalArgumentException("Illegal delay to start the sTimerTask: " + when);
            }
