...
public:
	...
	HJPluginAudioOHCapturer(const std::string& i_name = "HJPluginAudioOHCapturer", size_t i_identify = 0, HJKeyStorage::Ptr i_graphInfo = nullptr)
		: HJPluginCapturer(i_name, i_identify, i_graphInfo) {}
	virtual ~HJPluginAudioOHCapturer() { done(); }
