...
#define HJ_PROFILE_AV1_MAIN 0
#define HJ_PROFILE_AV1_HIGH 1
#define HJ_PROFILE_AV1_PROFESSIONAL 2
	...
	int* temporal_id, int* spatial_id)
{
	...
	ret = init_get_bits8(&gb, buf, HJ_MIN(buf_size, MAX_OBU_HEADER_SIZE));
	if (ret < 0)
		return ret;
...
static int parse_color_config(AV1SequenceParameters* seq_params, Av1GetBitContext* gb)
{
	...
	if (seq_params->profile == HJ_PROFILE_AV1_PROFESSIONAL && high_bitdepth)
		twelve_bit = get_bits(gb, 1);

	seq_params->bitdepth = 8 + (high_bitdepth * 2) + (twelve_bit * 2);

	if (seq_params->profile == HJ_PROFILE_AV1_HIGH)
	...
	if (seq_params->monochrome) {
		...
		if (seq_params->profile == HJ_PROFILE_AV1_MAIN) {
			seq_params->chroma_subsampling_x = 1;
			seq_params->chroma_subsampling_y = 1;
		}
		else if (seq_params->profile == HJ_PROFILE_AV1_HIGH) {
			seq_params->chroma_subsampling_x = 0;
			seq_params->chroma_subsampling_y = 0;
		}
		else {
...
HJBuffer::Ptr HJESAV1Parser::parseHeader(const uint8_t* data, size_t size)
{
	if (data[0] & 0x80) {
	...
	buffer->write(av1header, sizeof(av1header));
	if (seq_size)
		buffer->write(seq, seq_size);
	if (meta_size)
		buffer->write(meta, meta_size);

	return buffer;
}

HJBuffer::Ptr HJESAV1Parser::serializeData(const uint8_t* data, size_t size, bool* is_keyframe, int* priority)
{
