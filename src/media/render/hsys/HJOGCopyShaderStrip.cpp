...
int HJOGCopyShaderStrip::init(int i_nFlag)
{
    ...
    do {
        ...
        if (m_nFlag & OGCopyShaderStripFlag_2D)
        {
            ...
            fragShader = (m_nFlag & OGCopyShaderStripFlag_AlphaLeftRight) ? HJOGShaderCommon::s_fragmentCopyShaderAlphaLeftRight : HJOGShaderCommon::s_fragmentCopyShader;
        }   
        else if (m_nFlag & OGCopyShaderStripFlag_OES)
        {
            m_textureStyle = TEXTURE_TYPE_OES;
            fragShader = (m_nFlag & OGCopyShaderStripFlag_AlphaLeftRight) ? HJOGShaderCommon::s_fragmentCopyShaderAlphaLeftRightOES : HJOGShaderCommon::s_fragmentCopyShaderOES;
        }        
        
        HJFLogi("Texture Style: {}", m_textureStyle == TEXTURE_TYPE_2D ? "2D" : "OES");
        
        m_shaderProgram = HJOGShaderProgram::Create();
        i_err = m_shaderProgram->init(HJOGShaderCommon::s_vertexCopyShader, fragShader);
        if (i_err < 0)
        {
            break;
        }
