...
public:
    ...
    ThreadSafeFunctionWrapper(napi_env env, napi_value jsCallback,
    ...
    // 调用JS回调
    napi_status Call(HJYJsonDocument* data, napi_threadsafe_function_call_mode mode = napi_tsfn_nonblocking) const {
        if (m_tsfn == nullptr) {
            return napi_invalid_arg;
        }
        auto str = data->getSerialInfo();
