...
public:
    ...
    virtual int draw(GLuint textureId, int i_mode, int srcw, int srch, int dstw, int dsth, float *texMat = nullptr, bool i_bYFlip = false, bool i_bXFlip = false)
    {
        return 0;
    }
    ...
    static HJOGBaseShader::Ptr createShader(int i_shaderType, bool i_bForceXMirror = false, bool i_bForceYMirror = false);
    
private:
    std::string m_insName = "";    
};

class HJOGCopyShaderStripCompat : public HJOGBaseShader
{
public:
    HJ_DEFINE_CREATE(HJOGCopyShaderStripCompat);

    HJOGCopyShaderStripCompat();
	virtual ~HJOGCopyShaderStripCompat();
    
    virtual int init(int i_nFlag = OGCopyShaderStripFlag_2D, bool i_bPreMultipleShader = true) override;
    virtual int draw(GLuint textureId, int i_mode, int srcw, int srch, int dstw, int dsth, float *texMat = nullptr, bool i_bYFlip = false, bool i_bXFlip = false) override;
    virtual int draw(GLuint textureId, float *vertexMat, float *texMat);
...
class HJOGCopyShaderStrip : public HJOGCopyShaderStripCompat
{
public:
    ...
    virtual int init(int i_nFlag = OGCopyShaderStripFlag_2D, bool i_bPreMultipleShader = true) override;
    virtual int draw(GLuint textureId, int i_mode, int srcw, int srch, int dstw, int dsth, float* texMat = nullptr, bool i_bYFlip = false, bool i_bXFlip = false) override;
    virtual int draw(GLuint textureId, float* vertexMat, float* texMat) override;
    virtual void release() override;
private:
};
...
class HJOGShaderFilterSplitScreenLROES : public HJOGCopyShaderStrip
{
public:
	HJ_DEFINE_CREATE(HJOGShaderFilterSplitScreenLROES);
	HJOGShaderFilterSplitScreenLROES() = default;
	virtual ~HJOGShaderFilterSplitScreenLROES() = default;
	virtual std::string shaderGetFragment() override
	{
		return s_fragmentShader;
	}
...
class HJOGShaderFilterSplitScreenLR2D : public HJOGCopyShaderStrip
{
public:
	HJ_DEFINE_CREATE(HJOGShaderFilterSplitScreenLR2D);
    HJOGShaderFilterSplitScreenLR2D() = default;
	virtual ~HJOGShaderFilterSplitScreenLR2D() = default;
	virtual std::string shaderGetFragment() override
	{
		return s_fragmentShader;
	}
