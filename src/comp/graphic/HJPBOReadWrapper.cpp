...
void HJPBOReadWrapper::priWrite(unsigned char* i_pBuffer, int i_width, int i_height)
{
    static HJYuvWriter::Ptr yuvFilter = nullptr;
    if (!yuvFilter)
    {
        yuvFilter = HJYuvWriter::Create();
        yuvFilter->init(m_savePath, i_width, i_height);
    }
	...
	do
	{
        ...
        if (bCreate)
        {
            ...
            m_pboReader->setReadCb([this](unsigned char *i_pBuffer, int i_width, int i_height)
            {
                ...
                if (!m_savePath.empty())
                {
                    priWrite(i_pBuffer, i_width, i_height);
                }
                if (m_cb)
                {
