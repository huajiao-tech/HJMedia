...
int HJOGRenderWindowBridge::s_targetSdkVersion = 20;

HJOGRenderWindowBridge::HJOGRenderWindowBridge()
{
    m_sdkVersion = OH_GetSdkApiVersion();
}
HJOGRenderWindowBridge::~HJOGRenderWindowBridge()
{
...
//    static int s_nAvaiableCnt = 0;
//    HJFLogi("priOnFrameAvailable enter: {}", s_nAvaiableCnt);
//    s_nAvaiableCnt++;
}
...
int HJOGRenderWindowBridge::init()
{
    ...
    do          
    {
        ...
        if (m_sdkVersion >= s_targetSdkVersion)
        {
			HJFLogi("{} OH_GetSdkApiVersion: {} >= targetVersion:{}, use drop buffer mode", m_insName, m_sdkVersion, s_targetSdkVersion);
            void* handle = dlopen(NULL, RTLD_LAZY);
            if (!handle)
            {
                HJFLoge("{} dlopen failed, skip drop buffer mode", m_insName);
            }
            else
            {
                using SetDropBufferModeFunc = int32_t (*)(OH_NativeImage*, bool);
                auto setDropBufferMode = reinterpret_cast<SetDropBufferModeFunc>(
                    dlsym(handle, "OH_NativeImage_SetDropBufferMode"));
                if (!setDropBufferMode)
                {
                    HJFLogi("{} symbol OH_NativeImage_SetDropBufferMode not found, skip", m_insName);
                    dlclose(handle);
                }
                else
                {
					HJFLogi("{} setDropBufferMode", m_insName);
                    i_err = setDropBufferMode(m_nativeImage, true);
                    dlclose(handle);
                    if (i_err < 0)
                    {
                        HJFLoge("{} set drop buffer mode failed i_err:{}", m_insName, i_err);
                        break;
                    }
                }
            }
		...
		{
			HJFLogi("{} OH_GetSdkApiVersion: {}	< targetVersion:{}, not support drop buffer mode", m_insName, m_sdkVersion, s_targetSdkVersion);
		}
...
int HJOGRenderWindowBridge::priUpdate()
{
    ...
    do
    {       
//      static int s_nUpdateCnt = 0;
//		HJFLogi("{}, priUpdate enter {}", m_insName, s_nUpdateCnt);
//      s_nUpdateCnt++;
        
        if (m_manulDriveCb && (m_sdkVersion < s_targetSdkVersion))
        {
            while (true)
            {
		...
		if (i_err != 0)
		{
            //no idle buffer is available NATIVE_ERROR_NO_BUFFER = 40601000,
			if (NATIVE_ERROR_NO_BUFFER == i_err)
			{
				i_err = 0; //
				//HJFLogw("Update surface image warning:40601000 {} m_texture:{} wdith:{} height:{}", i_err, m_texture, m_srcWidth, m_srcHeight);
			}
			else
			{
                HJFLoge("Update surface image this:{} error:{} m_texture:{}", size_t(this), i_err, m_texture);
		...
		else
		{
            ...
            {
                HJFLoge("HVSwitchStat OH_NativeWindow_NativeWindowHandleOpt getwh w:{} h:{} error <producer, pixel, hwdecode, camera must SET_BUFFER_GEOMETRY > timestamp:{} ", m_srcWidth, m_srcHeight, timeStamp);
            }
            
            // static int s_tw = 0;
            // static int s_th = 0;
            // if (s_tw != m_srcWidth || s_th != m_srcHeight)
            // {
            //     s_tw = m_srcWidth;
            //     s_th = m_srcHeight;
            //     HJFLogi("HVSwitchStat OH_NativeImage_UpdateSurfaceImage w:{} h:{}", m_srcWidth, m_srcHeight);
            // }
...
void HJOGRenderWindowBridge::setPreviewRotation(int i_rotation)
{
