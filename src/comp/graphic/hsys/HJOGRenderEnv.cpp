...
{       
    m_insName = insName + "_" + std::to_string(m_debugIdx);
}
...
int HJOGRenderEnv::priUpdateEglSurface(const std::string& i_renderTargetInfo, std::shared_ptr<HJOGEGLSurface>& o_eglSurface)
{
 int i_err = 0;
    do
    {
        ...
        i_err = targetInfo.deserial(i_renderTargetInfo);
        if (i_err < 0)
        {
        ...
        if (nState == HJTargetState_Destroy)
        {
            for (HJOGEGLSurfaceQueueIt it = m_eglSurfaceQueue.begin(); it != m_eglSurfaceQueue.end(); ++it)
            {
                ...
                {
                    HJFLogi("{} renderThread destroy window enter", m_insName);
                    //o_eglSurface = *it;
                    o_eglSurface = nullptr;
        ...
        {    
			...
			if (window)
			{
                ...
                HJFLogi("HVSwitchStat EGL set target w:{} h:{}", targetWidth, targetHeight);
            ...
            else if (nState == HJTargetState_Change)
            {
                for (HJOGEGLSurfaceQueueIt it = m_eglSurfaceQueue.begin(); it != m_eglSurfaceQueue.end(); ++it)
                {
                    if ((*it)->getWindow() == window)
                    {
                        HJFLogi("HVSwitchStat EGL change set viewport target w:{} h:{}", targetWidth, targetHeight);
...
int HJOGRenderEnv::makeOffScreenCurrent()
{
