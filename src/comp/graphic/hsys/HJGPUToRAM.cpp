...
namespace
{
bool priShouldScaleFaceDetectInput(int i_width, int i_height)
{
    return ((i_width >= 720) && (i_height >= 1280)) ||
           ((i_width >= 1280) && (i_height >= 720));
}
}
...
void HJBaseGPUToRAM::setMediaData(HJSPBuffer::Ptr i_buffer, int width, int height)
{
    ...
    do
    {
        if (!i_buffer || width <= 0 || height <= 0)
        {
            ptr = HJRGBAMediaData::Create<HJRGBAMediaData>(i_buffer, width, height);
            break;
        }

        if (!priShouldScaleFaceDetectInput(width, height))
        {
            ptr = HJRGBAMediaData::Create<HJRGBAMediaData>(i_buffer, width, height, width, height);
            break;
        }

        const int scaleWidth = std::max(1, width / 2);
        const int scaleHeight = std::max(1, height / 2);
        HJSPBuffer::Ptr scaleBuffer = HJSPBuffer::create(scaleWidth * scaleHeight * 4);
        if (!scaleBuffer)
        {
            HJFLoge("{} create scaleBuffer failed src:{}x{} dst:{}x{}", __FUNCTION__, width, height, scaleWidth, scaleHeight);
            ptr = HJRGBAMediaData::Create<HJRGBAMediaData>(i_buffer, width, height, width, height);
            break;
        }

        const int ret = libyuv::ARGBScale(
            i_buffer->getBuf(), width * 4, width, height,
            scaleBuffer->getBuf(), scaleWidth * 4, scaleWidth, scaleHeight,
            libyuv::kFilterBilinear);
        if (ret != 0)
        {
            HJFLoge("{} ARGBScale failed ret:{} src:{}x{} dst:{}x{}", __FUNCTION__, ret, width, height, scaleWidth, scaleHeight);
            ptr = HJRGBAMediaData::Create<HJRGBAMediaData>(i_buffer, width, height, width, height);
            break;
        }

        ptr = HJRGBAMediaData::Create<HJRGBAMediaData>(scaleBuffer, width, height, scaleWidth, scaleHeight);
    } while (false);
...
{
    HJRGBAMediaData::Ptr data = HJRGBAMediaData::Create<HJRGBAMediaData>(i_width, i_height, i_width, i_height, i_data, i_stride, bufferSize);
    HJ_LOCK(m_mediaMutex)
    m_mediaQueue.clear();
