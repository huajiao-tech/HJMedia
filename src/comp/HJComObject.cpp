...
bool HJBaseCom::isNextFull(HJBaseCom::Ptr i_curCom)
{
    bool bAllFull = false;
    do
    {
        ...
        for (auto it = nextQueue.begin(); it != nextQueue.end(); it++)
        {
            ...
            if (com)
            {
                ...
                if (com->isDirectCom())
                {
                    bSingleFull = isNextFull(com);
                }
                else
                {
                    bSingleFull = com->isMediaQueueFull();
                }
                if (bSingleFull)
                {
                    i++;
...
void HJBaseComAsyncThread::setInsName(const std::string& i_insName)
{
    ...
    m_threadPool->setInsName(i_insName + "_thread");
...
{
    HJBaseCom::setInsName(i_insName);
    m_threadTimer->setInsName(i_insName + "_thread");
...
int HJBaseRenderCom::renderUpdate(HJBaseParam::Ptr i_param, SLComSendType i_sendType)
{
	int i_err = 0;
	do
	{
		if (i_sendType == SEND_TO_NEXT)
		{
			for (auto it = m_nextWeakComQueue.begin(); it != m_nextWeakComQueue.end(); ++it)
			{
				...
				if (com)
				{
					i_err = com->doRenderUpdate(i_param);
                    if (i_err < 0)
                    {
                        break;
                    }
...
int HJBaseRenderCom::renderDraw(HJBaseParam::Ptr i_param, SLComSendType i_sendType)
{
	int i_err = 0;
	do
	{
		if (i_sendType == SEND_TO_NEXT)
		{
			for (auto it = m_nextWeakComQueue.begin(); it != m_nextWeakComQueue.end(); ++it)
			{
				...
				if (com)
				{
					i_err = com->doRenderDraw(i_param);
                    if (i_err < 0)
                    {
                        break;
                    }
...
int HJBaseRenderCom::doRenderUpdate(HJBaseParam::Ptr i_param)
{
    return 0;
}
int HJBaseRenderCom::doRenderDraw(HJBaseParam::Ptr i_param)
{
    return 0;
}

bool HJBaseRenderCom::renderModeIsContain(int i_targetType)
{
    return (m_renderMap.find(i_targetType) != m_renderMap.end());
}
void HJBaseRenderCom::renderModeClear(int i_targetType)
{
...
void HJBaseRenderCom::renderModeClearAll()
{
    m_renderMap.clear();
    renderModeAdd(HJOGEGLSurfaceType_Default);
}
void HJBaseRenderCom::renderModeAdd(int i_targetType, HJTransferRenderModeInfo::Ptr i_rendermMode)
{
...
std::vector<HJTransferRenderModeInfo::Ptr>& HJBaseRenderCom::renderModeGet(int i_targetType)
{
